...
class LipidsPage extends StatefulWidget {
  @override
  _LipidsPageState createState() => _LipidsPageState();
  final InfoServices AdviceService = InfoServices();
}

class _LipidsPageState extends State<LipidsPage> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      stream: postsCollection.where('category', isEqualTo: 'Lipids').snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (posts.isNotEmpty) {
            ...
            return Center(child: Text('No Lipids information found.'));
          }
        }
      },
    );
