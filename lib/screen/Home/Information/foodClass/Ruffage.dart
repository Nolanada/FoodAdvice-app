...
class RuffagePage extends StatefulWidget {
  @override
  _RuffagePageState createState() => _RuffagePageState();
  final InfoServices AdviceService = InfoServices();
}

class _RuffagePageState extends State<RuffagePage> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      stream: postsCollection.where('category', isEqualTo: 'Ruffage').snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (posts.isNotEmpty) {
            ...
            return Center(child: Text('No Ruffage information found.'));
          }
        }
      },
    );
