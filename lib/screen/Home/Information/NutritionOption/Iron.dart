...
class IronPage extends StatefulWidget {
  @override
  _IronPageState createState() => _IronPageState();
...
class _IronPageState extends State<IronPage> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      stream: postsCollection.where('category', isEqualTo: 'Iron').snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (posts.isNotEmpty) {
            ...
            return Center(child: Text('No Iron information found.'));
          }
        }
      },
    );
